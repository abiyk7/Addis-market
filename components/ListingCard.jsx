...
export default function ListingCard({ listing }) {
  ...
  return (
    <a href={`/listing/${listing.id}`} className="rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow flex flex-col" style={{ background: COLORS.card, border: `1px solid ${COLORS.parchmentDark}` }}>
      <div className="h-36 relative flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${COLORS.coffee}dd, ${COLORS.coffeeDark})` }}>
        ...
        {listing.profiles?.is_verified_seller && (
          <span className="absolute bottom-2 left-2 flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full" style={{ background: COLORS.forest, color: COLORS.parchment }}>
            <ShieldCheck size={11} /> የተረጋገጠ
          </span>
        )}
      </div>
      <div className="p-3 flex flex-col gap-1 flex-1">
        <span className="text-[11px] font-semibold" style={{ color: COLORS.goldDark }}>{cat.amh}</span>
        <h3 className="font-bold text-sm leading-snug line-clamp-2">{listing.title}</h3>
        <span className="text-lg font-bold mt-1" style={{ fontFamily: "'IBM Plex Mono', monospace", color: listing.currency === "በነፃ" ? COLORS.forest : COLORS.rust }}>
        ...
        <div className="text-[11px] mt-1" style={{ color: COLORS.inkSoft }}>{timeAgo(listing.created_at)}</div>
      </div>
    </a>
  );
}
