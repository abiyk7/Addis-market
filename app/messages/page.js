  ...
  for (const m of messages || []) {
    ...
    if (!conversations.has(key)) {
      conversations.set(key, {
        ...
        listingTitle: m.listings?.title || "ማስታወቂያ",
        otherId,
        lastMessage: m.body,
        lastAt: m.created_at,
      });
  ...
  return (
    <div className="min-h-screen py-8 px-4" style={{ background: COLORS.parchment }}>
      <div className="max-w-lg mx-auto">
        <h1 className="text-xl font-bold mb-4" style={{ color: COLORS.coffeeDark }}>መልእክቶች · Messages</h1>

        {list.length === 0 && (
          <p className="text-sm text-center py-10" style={{ color: COLORS.inkSoft }}>ገና ምንም ውይይት የለም።</p>
        )}
