    ...
    <div style={{ minHeight: "100vh", background: "#FFFDF7" }}>
      ...
      <header className="sticky top-0 z-30" style={{ background: "#FFFDF7", borderBottom: `1px solid ${COLORS.parchmentDark}` }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4 flex-wrap">
          <a href="/" className="flex items-baseline gap-2 shrink-0">
            <span className="text-2xl font-bold" style={{ color: COLORS.goldDark }}>{t.siteNameNative}</span>
            <span className="text-xs uppercase tracking-widest opacity-70" style={{ fontFamily: "'Fraunces', serif", color: COLORS.coffee }}>{t.tagline}</span>
          </a>
          ...
          <LanguageToggle lang={lang} label={t.langToggle} />
          <AuthStatus />
        </div>
      </header>
        ...
        >
          ...
          <div className="flex justify-center gap-3 mt-4 flex-wrap text-xs font-semibold">
            <span className="flex items-center gap-1 px-3 py-1.5 rounded-full" style={{ background: COLORS.forest, color: "#FFFDF7" }}>
              <ShieldCheck size={14} /> {t.badgeSecure}
            </span>
            <span className="px-3 py-1.5 rounded-full" style={{ background: COLORS.gold, color: COLORS.coffeeDark }}>{t.badgeFree}</span>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-4 overflow-x-auto">
        <div className="flex gap-2 min-w-max">
          ...
          >
            <span className="text-lg">🛒</span> {t.catAll}
          </a>
          {CATEGORIES.map((c) => (
            <a
              key={c.id}
            ...
            >
              <span className="text-lg">{c.emoji}</span> {lang === "am" ? c.amh : (c.en || c.amh)}
            </a>
          ))}
        </div>
      </section>
      ...
      <main className="max-w-6xl mx-auto px-4 pb-16">
        <h2 className="text-lg font-bold mb-3" style={{ color: COLORS.coffeeDark }}>{t.recentListings}</h2>

        {error && <p className="text-sm" style={{ color: COLORS.rust }}>{t.loadError}</p>}

        {!error && (!listings || listings.length === 0) && (
          <div className="text-center py-16 rounded-2xl" style={{ background: COLORS.card, border: `1px dashed ${COLORS.parchmentDark}` }}>
            <p className="font-semibold mb-2">{t.noResults}</p>
      ...
      <footer className="text-center py-6 text-xs px-4" style={{ background: "#FFFDF7", color: COLORS.inkSoft, borderTop: `1px solid ${COLORS.parchmentDark}` }}>
        <p className="font-semibold" style={{ fontFamily: "'Fraunces', serif", color: COLORS.coffeeDark }}>{t.siteNameNative} · Addis Market</p>
        <p className="opacity-70 mt-1">{t.footerTagline}</p>
        <p className="mt-3 max-w-xl mx-auto leading-relaxed" style={{ opacity: 0.75 }}>
          {t.footerDisclaimer}
        </p>
        <p className="mt-3">
          <a href="/privacy" className="font-semibold underline" style={{ color: COLORS.goldDark }}>{t.privacyPolicy}</a>
        </p>
      </footer>
    </div>
  );
